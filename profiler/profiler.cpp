...
print_results(INT32 code, void *v)
{
  ...
  for(i = cflows.begin(); i != cflows.end(); i++) {
    ...
    for(j = i->second.begin(); j != i->second.end(); j++) {
      ip = j->first;
      count = j->second;
      printf("0x%08px <- 0x%08px: %3lu (%0.2f%%)\n", 
             (void *)target, (void *)ip, count, (double)count/cflow_count*100.0);
    } 
  }
  ...
  if(!calls.empty()) {
    ...
    for(i = calls.begin(); i != calls.end(); i++) {
      ...
      for(j = i->second.begin(); j != i->second.end(); j++) {
        ip = j->first;
        count = j->second;
        printf("[%-30s] 0x%08px <- 0x%08px: %3lu (%0.2f%%)\n", 
               funcnames[target].c_str(), (void *)target, (void *)ip, count, (double)count/call_count*100.0);
      } 
    }
  }
  ...
  if(!syscalls.empty()) {
    ...
    for(j = syscalls.begin(); j != syscalls.end(); j++) {
      count = j->second;
      printf("%3pu: %3lu (%0.2f%%)\n", (void *)j->first, count, (double)count/syscall_count*100.0);
    }
  }
}
