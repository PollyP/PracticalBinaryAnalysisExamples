...
mem_to_file(mem_cluster_t *c, ADDRINT entry)
{
  ...
  fprintf(logfile, "extracting unpacked region 0x%016px (%9s) %s%s entry 0x%016px\n", 
          (void *)c->base, buf, c->w ? "w" : "-", c->x ? "x" : "-", (void *)entry);

  snprintf(buf, sizeof(buf), "unpacked.0x%px-0x%px_entry-0x%px", 
           (void *)c->base, (void *)(c->base+c->size), (void *)entry);
  ...
  if(!f) {
    ...
    for(ADDRINT i = c->base; i < c->base+c->size; i++) {
      if(fwrite((const void*)&shadow_mem[i].val, 1, 1, f) != 1) {
        fprintf(logfile, "failed to write unpacked byte 0x%px to file '%s'\n", (void *)i, buf);
      }
    }
    fclose(f);
  }
...
print_clusters()
{
  ...
  for(j = 0; j < clusters.size(); j++) {
    ...
    fprintf(logfile, "0x%016px (%9s) %s%s: ", 
            (void *)clusters[j].base, buf,
